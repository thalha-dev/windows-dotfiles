# syncup.ps1
# Sync local art folder to Google Drive (local -> gdrive:art)
$LocalArtPath = "D:\workspace\personal\art"
...
if (-not (Test-Path $LocalArtPath)) {
    Write-Host "ERROR: Local folder not found: $LocalArtPath" -ForegroundColor Red
    exit 2
}
...
$dryArgs = @("sync", $LocalArtPath, $RemoteArt, "--dry-run", "--progress")
Write-Host "DRY-RUN: what would change (local -> drive)" -ForegroundColor Cyan
$dryOut = & rclone @dryArgs 2>&1
Write-Host $dryOut
$dryLog = Save-Log "syncup-dry" $dryOut
Write-Host "Dry-run saved: $dryLog" -ForegroundColor Yellow

do {
    $ans = Read-Host "Proceed with actual sync (this WILL update Drive to match local)? (y/n)"
    $ans = $ans.Trim().ToLower()
} while ($ans -ne 'y' -and $ans -ne 'n')

if ($ans -eq 'y') {
    $realArgs = @("sync", $LocalArtPath, $RemoteArt, "--progress")
    Write-Host "Running actual sync..." -ForegroundColor Green
    $realOut = & rclone @realArgs 2>&1
    Write-Host $realOut
    $realLog = Save-Log "syncup-real" $realOut
